...
ptr<buffer> log_state_machine::commit(const ulong log_idx, buffer &data) {
  std::string content(reinterpret_cast<const char *>(data.data() + data.pos()),
                      data.size());
  std::cout << "提交日志条目: " << log_idx << ", 内容: " << content
            << std::endl;
  return nullptr;
}
...
void log_state_machine::snapshot_save(const ptr<snapshot> &s,
                                      const ulong log_idx) {}
...
void log_state_machine::create_snapshot(
    snapshot &s,
