...
SimpleLoggerMgr::TimeInfo::TimeInfo(std::tm *src)
    : year(src->tm_year + 1900), month(src->tm_mon + 1), day(src->tm_mday),
      hour(src->tm_hour), min(src->tm_min), sec(src->tm_sec), msec(0), usec(0) {
}
...
#if defined(__linux__) || defined(__APPLE__)
  size_t len =
      _stack_interpret(&stack_info.stackPtrs[0], stack_info.stackPtrs.size(),
...
void SimpleLoggerMgr::logStackBacktrace(size_t timeout_ms) {
  ...
  if (!crashDumpPath.empty() && !crashDumpFile.is_open()) {
    ...
    snprintf(time_fmt, 64, "%04d-%02d-%02dT%02d:%02d:%02d.%03d%03d%c%02d:%02d",
             ...
             lt.usec, (tz_gap >= 0) ? '+' : '-', (int)(tz_gap_abs / 60),
             tz_gap_abs % 60);
...
SimpleLoggerMgr::SimpleLoggerMgr()
    : termination(false), oldSigSegvHandler(nullptr),
      oldSigAbortHandler(nullptr), stackTraceBuffer(nullptr),
      crashOriginThread(0), crashDumpOriginOnly(true), exitOnCrash(false),
      abortTimer(0) {
#if defined(__linux__) || defined(__APPLE__)
  std::string env_segv_str;
...
SimpleLogger::SimpleLogger(const std::string &file_path, size_t max_log_elems,
    ...
    : filePath(replaceString(file_path, "//", "/")), maxLogFiles(max_log_files),
      maxLogFileSize(log_file_size_limit), numCompJobs(0), curLogLevel(4),
      curDispLevel(4), tzGap(SimpleLoggerMgr::getTzGap()), cursor(0),
      logs(max_log_elems) {
...
int SimpleLogger::start() {
  ...
  _log_sys(ll, "Start logger: %s (%zu MB per file, up to %zu files)",
           filePath.c_str(), maxLogFileSize / 1024 / 1024, maxLogFiles.load());
...
void SimpleLogger::put(int level, const char *source_file,
  ...
  _snprintf(msg, avail_len, cur_len, msg_len,
            ...
            "[%s] ",
            lt.year, lt.month, lt.day, lt.hour, lt.min, lt.sec, lt.msec,
            lt.usec, (tzGap >= 0) ? '+' : '-', tz_gap_abs / 60, tz_gap_abs % 60,
            TID_DIGITS, tid_hash, lv_names[level]);
#else
  _snprintf(msg, avail_len, cur_len, msg_len,
            "%04d-%02d-%02dT%02d:%02d:%02d.%03d_%03d%c%02d:%02d "
            "[%04x] "
  ...
  if (source_file && func_name) {
    _snprintf(msg, avail_len, cur_len, msg_len, "\t[%s:%zu, %s()]\n",
              source_file + ((last_slash) ? (last_slash + 1) : 0), line_number,
              func_name);
  } else {
    _snprintf(msg, avail_len, cur_len, msg_len, "\n");
  }
...
#ifdef __linux__
  _snprintf(
      msg, avail_len, cur_len, msg_len,
      " [" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") "." _CL_BROWN(
          "%03d") " " _CL_BROWN("%03d") "] [tid " _CL_B_BLUE("%*u") "] "
                                                                    "[%s] ",
      lt.hour, lt.min, lt.sec, lt.msec, lt.usec, TID_DIGITS, tid_hash,
  ...
  _snprintf(
      ...
      " [" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") "." _CL_BROWN(
          "%03d") " " _CL_BROWN("%03d") "] [tid " _CL_B_BLUE("%04x") "] "
                                                                     "[%s] ",
      lt.hour, lt.min, lt.sec, lt.msec, lt.usec, tid_hash,
      colored_lv_names[level]);
#endif
